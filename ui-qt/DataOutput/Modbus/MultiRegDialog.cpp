...
#include <QDebug>
#include <QMessageBox>
#include "MultiRegItem.h"
#include "stdio.h"
MultiRegDialog::MultiRegDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::MultiRegDialog)
{
    ui->setupUi(this);
    Ini_Flag = 0;
    LoadKeyBoardLib();
...
void MultiRegDialog::IniRegData()
{
...
void MultiRegDialog::GetRegData()
{
    Ini_Flag = 1;
    ...
    if(slave_or_host == MODBUS_SLAVE_TYPE)
    {
        ...
        {
            modbus_data_list_get(Data_List_Index, &S_WR_Hold_Reg_Data);
        ...
        {
            modbus_data_list_get(Data_List_Index, &S_WR_Input_Reg_Data);
    ...
    else
    {
        ...
        {
            modbus_data_list_get(Data_List_Index, &H_WR_Hold_Reg_Data);
...
void MultiRegDialog::SetDataListIndex(int List_Index)
{
    Data_List_Index = List_Index;
}

void MultiRegDialog::LoadKeyBoardLib()
{
...
void MultiRegDialog::NumKeyBoardDlg(QObject *watched, QEvent *event, int Min_value, int Max_value)
{
...
void MultiRegDialog::SetModbusMode(int mode)
{
    slave_or_host = mode;
}

void MultiRegDialog::SetToolType(int tool)
{
    tool_type = tool;
}

bool MultiRegDialog::eventFilter(QObject *watched, QEvent *event)
{
...
void MultiRegDialog::AddMultiRegItem(MultiRegItem *item)
{
...
void MultiRegDialog::RegSetChanged()
{
    if(Ini_Flag == 0)
    {
...
void MultiRegDialog::RegCofingChanged(int number, int coil_addr, int data_cnts)
{
    ui->listWidget_Reg->clear();
    for(int i = 1; i <= data_cnts; i++)
    {
        MultiRegItem * item = new MultiRegItem;
...
void MultiRegDialog::CalcPageData()
{
    int length = ui->lineEdit_Length->text().toInt();
    if(length > 8)
    {
...
void MultiRegDialog::on_pushButton_pre_clicked()
{
    if(current_page == 1)
    {
        return;
...
void MultiRegDialog::on_pushButton_next_clicked()
{
...
void MultiRegDialog::on_lineEdit_StartAddr_textChanged(const QString &arg1)
{
    RegSetChanged();
}

void MultiRegDialog::on_lineEdit_Length_textChanged(const QString &arg1)
{
    RegSetChanged();
    CalcPageData();
}

void MultiRegDialog::on_comboBox_DataType_currentIndexChanged(int index)
{
    Ini_Flag = 0;
    RegSetChanged();
    Ini_Flag = 1;
...
void MultiRegDialog::on_btn_Finish_clicked()
{
    if(slave_or_host == MODBUS_SLAVE_TYPE)
    {
        if(tool_type == SLAVE_WR_HOLD_REG)
        {
            ...
            modbus_data_list_set(MODBUS_SLAVE_TYPE, Data_List_Index, &S_WR_Hold_Reg_Data, SLAVE_WR_HOLD_REG);
        }
        else if(tool_type == SLAVE_WR_INPUT_REG)
        {
            ...
            modbus_data_list_set(MODBUS_SLAVE_TYPE, Data_List_Index, &S_WR_Input_Reg_Data, SLAVE_WR_INPUT_REG);
        }
    }
    else
    {
        if(tool_type == HOST_WR_HOLD_REG)
        {
            ...
            modbus_data_list_set(MODBUS_SLAVE_TYPE, Data_List_Index, &H_WR_Hold_Reg_Data, HOST_WR_HOLD_REG);
        }
    }
...
void MultiRegDialog::on_btn_Quit_clicked()
{
    QDialog::reject();
}
