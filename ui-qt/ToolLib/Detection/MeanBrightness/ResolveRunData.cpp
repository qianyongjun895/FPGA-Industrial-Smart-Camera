...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    MEANBRI_OUTPUT_PARAM* pRunPara = (MEANBRI_OUTPUT_PARAM*)Task_Step_Output_Para_Get(step);
    STRU_KEY_VALUE value;
    value.strName = "任务名称:平均亮度";
    value.strValue = "\n";
    vt.append(value);
...
QVector<STRU_KEY_VALUE> ResolveRunData::GetContent(int step, int index, int *count)
{
    *count = 1;
    ...
    info.strName = "平均亮度:";
    info.strValue = QString::number(pRstData[MEAN_BRIGHTNESS_ID].value.U32_Value);
    vt.append(info);
    info.strName = " ";
    info.strValue = " ";
    vt.append(info);
    ...
    info.strName = "像素数:";
    info.strValue = QString::number(pRstData[MEAN_BRIGHTNESS_PIXEL_CNT_ID].value.U32_Value);
    vt.append(info);
    return vt;
}
