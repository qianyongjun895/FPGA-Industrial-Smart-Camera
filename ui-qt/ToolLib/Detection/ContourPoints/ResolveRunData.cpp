...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    OLPXNUM_OUTPUT_PARAM* pRunPara = (OLPXNUM_OUTPUT_PARAM*)Task_Step_Output_Para_Get(step);

    STRU_KEY_VALUE value;
    value.strName = "任务名称:轮廓点数";
    value.strValue = "\n";
    vt.append(value);
...
QVector<STRU_KEY_VALUE> ResolveRunData::GetContent(int step, int index, int *count)
{
    *count =1;
    ...
    QVector<STRU_KEY_VALUE> vt;
    info.strName = "轮廓点数:";
    info.strValue = QString::number(pRstData[OLPXNUM_ID].value.U32_Value);
    vt.append(info);
    return vt;
}
