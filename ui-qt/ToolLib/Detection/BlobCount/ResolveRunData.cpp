...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    BLOBCOUNT_OUTPUT_PARAM* pRunPara = (BLOBCOUNT_OUTPUT_PARAM*)Task_Step_Output_Para_Get(step);

    STRU_KEY_VALUE value;
    value.strName = "任务名称:Blob计数";
    value.strValue = "\n";
    vt.append(value);
...
QVector<STRU_KEY_VALUE> ResolveRunData::GetContent(int step,int index,int *count)
{
    ...
    QVector<STRU_KEY_VALUE> vt;
    *count =1;
//    if(pRstData[BLOB_COUNT_OK_NG_ID].value.I32_Value)
    {
        info.strName = "Blob数量:";
        info.strValue = QString::number(pRstData[BLOB_NUM].value.U32_Value);
        vt.append(info);
    }
    return vt;
}
