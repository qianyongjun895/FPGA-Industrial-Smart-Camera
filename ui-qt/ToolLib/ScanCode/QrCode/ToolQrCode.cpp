...
#include "ui_ToolQrCode.h"
#include "FWQRCodeLabel.h"
#include <QDebug>
#include <QMessageBox>
ToolQrCode::ToolQrCode(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::ToolQrCode)
{
    ui->setupUi(this);
    LoadKeyBoardLib();
    m_label = new FWQRCodeLabel(this);
    ...
    ui->comboBox_Code_Type->addItem(tr("QR Code"));
    ui->comboBox_Code_Type->addItem(tr("DataMatrix Code"));
    ui->comboBox_Code_Type->addItem(tr("PDF417 Code"));

    for(int i = 0; i < 8; i++){
        ui->comboBox_scale->addItem(QString::number(i+1));
    }
...
void ToolQrCode::IniData()
{
    ui->comboBox_Code_Type->setCurrentIndex(pInputPara->Code_Type);
    if(pInputPara->Code_Type != 1){
        ui->label_scale->setVisible(false);
        ui->comboBox_scale->setVisible(false);
    }else{
        ui->label_scale->setVisible(true);
        ui->comboBox_scale->setVisible(true);
        ui->comboBox_scale->setCurrentIndex(pInputPara->scale - 1);
    }
    if(pInputPara->Read_Data_Flag == 1){
        ...
        for(int i = 0; i < Segmentation_Data_Num; i++){
...
void ToolQrCode::Init_Input_Ptr(void *pInpuPara,int i_step_index ,int new_flag,void *pen_color)
{
    ((FWQRCodeLabel*)m_label)->SetPenColor(pen_color);
    pInputPara = (QR_CODE_INPUT_PARAM*)pInpuPara;
    ...
    if(new_flag){
        ...
        pInputPara->scale = 8;
        ui->comboBox_scale->setCurrentIndex(7);
        pInputPara->Fail_Config.Fail_Output_Flag = 0;
        pInputPara->Cmp_Config.Cmp_Type = -1;
        memset(pInputPara->Cmp_Config.Cmp_Data, 0, CMP_DATA_LENGTH);
    }
    IniData();
}

int ToolQrCode::CheckSetParam()
{
...
int ToolQrCode::Set_Parameter_To_Ram()
{
    ...
    if(pInputPara->Code_Type == 1){
        pInputPara->scale = ui->comboBox_scale->currentText().toUInt();
    }
    if(ui->lineEdit_Data_Start_Bit->text() == "---" && ui->lineEdit_Data_Length->text() == "---"){
        pInputPara->Read_Data_Flag = 0;
    }else{
    ...
    if(ui->checkBox_fail_Output->isChecked()){
        ...
        memset(pInputPara->Fail_Config.Fail_Output_Data, 0, FAIL_OUTPUT_LENGTH);
...
QPoint ToolQrCode::Get_Image_Pos()
{
    QPoint point(10,10);
...
bool ToolQrCode::eventFilter(QObject *watched, QEvent *event)
{
    if(watched == ui->lineEdit_Data_Start_Bit || watched == ui->lineEdit_Data_Length){
        if(event->type() == QEvent::MouseButtonRelease){
            if(((QLineEdit*)watched)->isEnabled()){
                if(ui->comboBox_Code_Type->currentIndex() == 1){
                    NumKeyBoardDlg(watched, 1, 1, 500);
                }else{
                    NumKeyBoardDlg(watched, 1, 1, CODE_RESULT_LENGTH);
                }
            }
        }
    }
    ...
    if(watched == ui->lineEdit_Fail_Output_String){
        if(event->type() == QEvent::MouseButtonRelease){
            if(((QLineEdit*)watched)->isEnabled()){
                KeyBoardDlg(watched, event, 0, 0, FAIL_OUTPUT_LENGTH);
            }
        }
    }
    if(watched == ui->lineEdit_Comp_Data){
        if(event->type() == QEvent::MouseButtonRelease){
            if(((QLineEdit*)watched)->isEnabled()){
                KeyBoardDlg(watched, event, 0, 150, CMP_DATA_LENGTH);
            }
        }
    }
...
void ToolQrCode::LoadKeyBoardLib()
{
...
void ToolQrCode::NumKeyBoardDlg(QObject *watched, int clear_en, int Min_value, int Max_value)
{
...
void ToolQrCode::KeyBoardDlg(QObject *watched, QEvent *event, int dlg_x, int dlg_y, int String_Len)
{
...
void ToolQrCode::on_lineEdit_Data_Start_Bit_textChanged(const QString &arg1)
{
    DataEnable();
}

void ToolQrCode::on_lineEdit_Data_Length_textChanged(const QString &arg1)
{
    DataEnable();
}

void ToolQrCode::on_checkBox_Data1_clicked()
{
...
void ToolQrCode::on_checkBox_Data2_clicked()
{
...
void ToolQrCode::on_checkBox_Data3_clicked()
{
...
void ToolQrCode::on_checkBox_Data4_clicked()
{
...
void ToolQrCode::on_checkBox_Output_Length_clicked()
{
    if(ui->checkBox_Output_Length->isChecked()){
        ui->lineEdit_fill->setEnabled(true);
        ui->label_fill->setEnabled(true);
...
void ToolQrCode::on_comboBox_Code_Type_currentIndexChanged(int index)
{
    if(index == 1){
        ui->label_scale->setVisible(true);
        ui->comboBox_scale->setVisible(true);
