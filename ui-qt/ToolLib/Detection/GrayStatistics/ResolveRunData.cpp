...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    pRunPara = (GRAY_STAT_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);
...
//    if(pRstData->resultflag)//该数据有效
    {
        info.strName = "像素:";
        info.strValue = QString::number(pRstData[GRAY_STAT_NUM].value.U32_Value);
        vt.append(info);
    }
    return vt;
}
