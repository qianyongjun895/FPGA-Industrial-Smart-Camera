...
bool frmInput::eventFilter(QObject *obj, QEvent *event)
{
    ...
    {
        qDebug()<<"currentWidget  = "<<__LINE__<<" "<< currentWidget->objectName();
        if(currentWidget->objectName() == obj->objectName())
        {
...
void frmInput::focusChanged(QWidget *oldWidget, QWidget *nowWidget)
{
    ...
    if (nowWidget != 0 && !this->isAncestorOf(nowWidget)) {
        ...
        isFirst = false;
        if(PannelJudge(nowWidget->objectName()))
        {
            if (nowWidget->inherits("QLineEdit")) {
//                if(nowWidget->objectName() == "txtPwd" || nowWidget->objectName() == "lineEdit_window")
//                {
//                    return;
//                }
...
void frmInput::clearFocusPro()
{
    if (currentEditType == "QLineEdit") {
        ...
        currentPlain->clearFocus();
        currentPlain = 0;
    } else if (currentEditType == "QTextBrowser") {
        currentBrowser->clearFocus();
        currentBrowser = 0;
    } else if (currentEditType == "QWidget") {
...
int frmInput::PannelJudge(QString id)
{
    QStringList NameList;
    NameList.append("lineEdit_TaskName");
    NameList.append("lineEdit_AuthorName");
    NameList.append("textEdit_TaskInfo");
    NameList.append("lineEdit_ImportName");
    NameList.append("lineEdit_CommonRowInput");
    NameList.append("txtModifyTaskName");
    NameList.append("txtToolRename");
//    NameList.append("txtPwd");
