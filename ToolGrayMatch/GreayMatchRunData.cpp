...
GreayMatchRunData::GreayMatchRunData()
{
   // pRunPara = NULL;
    m_ok_cnt = 0;
    m_ng_cnt = 0;
...
//对外颜色设置模块
void GreayMatchRunData::Paint(QPainter *paint,int step,void *penStyle,int paintAreaSelect)
{
 ...
 * @note 获取
 */
QVector<STRU_KEY_VALUE> GreayMatchRunData::GetHead(int step)
{
    QVector<STRU_KEY_VALUE> vt;
   // pRunPara = (GREAY_MATCH_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);  //得到算法执行的输出接口
...
QVector<STRU_KEY_VALUE> GreayMatchRunData::GetContent(int step, int index, int *count)  //持续调用的函数
{
    *count =1;
...
//    if(pRstData->resultflag)//该数据有效
    {
        info.strName = "面积:";
      //  info.strValue = QString::number(pRstData[MONO_AREA].value.U32_Value);
        vt.append(info);
    }
    return vt;
}

void GreayMatchRunData::GetPreData(QPainter *paint, int iStep,int iSwitch)
{
...
/*void GreayMatchRunData::GetRoiData(QPainter *paint,SELECT_AREA_PARAM del_model)
{
...
void GreayMatchRunData::DrawPolygon(QPainter *paint, OCTAGON_CFG_PARA points)
{
...
void GreayMatchRunData::DrawRing(QPainter *paint, CIRCULAR ring)
{
