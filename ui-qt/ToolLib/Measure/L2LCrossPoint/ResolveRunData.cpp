...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    L2L_CROSS_POINT_OUTPUT_PARAM *pRunPara = (L2L_CROSS_POINT_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);
    STRU_KEY_VALUE info;
    info.strName = "任务名称:两线交点";
    info.strValue = "\r\n";
    vt.append(info);
...
//    if(pRstData->resultflag)//该数据有效
    {
        ...
        float pix_sub_x = 0.0;
        float pix_sub_y = 0.0;
        info.strName = "交点坐标(像素):";
        pix_sub_x = pRstData[LLCP_CROSS_POINT_X_ID].value.F32_Value;
        pix_sub_y = pRstData[LLCP_CROSS_POINT_Y_ID].value.F32_Value;
        PixelCvt(&pix_sub_x,&pix_vga_x,CVT_SUB2VGA|CVT_F2I|CVT_DATA);
        PixelCvt(&pix_sub_y,&pix_vga_y,CVT_SUB2VGA|CVT_F2I|CVT_DATA);
        PixelCvt(&pix_sub_x,&real_pix_x,CVT_SUB2REAL|CVT_F2I|CVT_DATA);
        PixelCvt(&pix_sub_y,&real_pix_y,CVT_SUB2REAL|CVT_F2I|CVT_DATA);
        m_point.setX(pix_vga_x);
        m_point.setY(pix_vga_y);
        info.strValue = "("+QString::number(real_pix_x)+","+\
                QString::number(real_pix_y)+")";
        vt.append(info);

        info.strName = " ";
        info.strValue = " ";
        vt.append(info);
        info.strName = "交点坐标(物理):";
        info.strValue = "("+QString::number(pRstData[LLCP_PHY_CROSS_POINT_X_ID].value.F32_Value)+","+\
                QString::number(pRstData[LLCP_PHY_CROSS_POINT_Y_ID].value.F32_Value)+")";
        vt.append(info);
