...
#include "ui_IntercomReadRegDialog.h"
...
IntercomReadRegDialog::IntercomReadRegDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::IntercomReadRegDialog)
{
    ui->setupUi(this);
    LoadKeyBoardLib();
    IniData();
...
IntercomReadRegDialog::~IntercomReadRegDialog()
{
...
void IntercomReadRegDialog::IniData()
{
...
void IntercomReadRegDialog::IniRegData()
{
...
void IntercomReadRegDialog::GetRegData()
{
    if(slave_or_host == MODBUS_SLAVE_TYPE)
    {
        if(tool_type == SLAVE_RD_HOLD_REG)
        {
            intercom_modbus_data_list_get(Data_List_Index, &S_RD_Hold_Reg_Data);
    ...
    else if(slave_or_host == MODBUS_HOST_TYPE)
    {
        ...
        {
            intercom_modbus_data_list_get(Data_List_Index, &H_RD_Hold_Reg_Data);
        ...
        {
            intercom_modbus_data_list_get(Data_List_Index, &H_RD_Input_Reg_Data);
    ...
    int index = intercom_modbus_list_exec_step_get(Data_List_Index) - 1;
    if(index >= 0)
...
void IntercomReadRegDialog::SetDataListIndex(int List_Index)
{
    Data_List_Index = List_Index;
}

void IntercomReadRegDialog::LoadKeyBoardLib()
{
...
void IntercomReadRegDialog::NumKeyBoardDlg(QObject *watched, QEvent *event, int Min_value, int Max_value)
{
...
void IntercomReadRegDialog::SetModbusMode(int mode)
{
    slave_or_host = mode;
}

void IntercomReadRegDialog::SetToolType(int tool)
{
    tool_type = tool;
}

bool IntercomReadRegDialog::eventFilter(QObject *watched, QEvent *event)
{
...
void IntercomReadRegDialog::on_btn_Finish_clicked()
{
    if(slave_or_host == MODBUS_SLAVE_TYPE)
    {
        if(tool_type == SLAVE_RD_HOLD_REG)
        {
            ...
            intercom_modbus_data_list_set(slave_or_host, Data_List_Index, &S_RD_Hold_Reg_Data, tool_type);
        }
    }
    else if(slave_or_host == MODBUS_HOST_TYPE)
    {
        if(tool_type == HOST_RD_HOLD_REG)
        {
            ...
            intercom_modbus_data_list_set(slave_or_host, Data_List_Index, &H_RD_Hold_Reg_Data, tool_type);
        }
        else if(tool_type == HOST_RD_INPUT_REG)
        {
            ...
            intercom_modbus_data_list_set(slave_or_host, Data_List_Index, &H_RD_Input_Reg_Data, tool_type);
        }
    }
    intercom_modbus_list_exec_step_set(Data_List_Index, ui->comboBox_TaskType->currentIndex()+1);
...
void IntercomReadRegDialog::on_btn_Cancel_clicked()
{
    QDialog::reject();
}
