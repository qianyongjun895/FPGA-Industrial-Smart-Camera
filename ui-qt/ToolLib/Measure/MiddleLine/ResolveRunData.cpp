...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    LLML_OUTPUT_PARAM *pRunPara = (LLML_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);
    STRU_KEY_VALUE info;
    info.strName = "任务名称:中分线";
    info.strValue = "\r\n";
    vt.append(info);
...
//    if(pRstData->resultflag)//该数据有效
    {
        info.strName = "弧度:";
        info.strValue = QString::number(pRstData[LLML_THETA_ID].value.D64_Value);
        vt.append(info);
        info.strName = " ";
        info.strValue = " ";
        vt.append(info);
        info.strName = "角度:";
        info.strValue = QString::number(pRstData[LLML_DEGREE_ID].value.D64_Value);
        vt.append(info);
        info.strName = " ";
        info.strValue = " ";
        vt.append(info);
        info.strName = "旋转角度:";
        info.strValue = QString::number(pRstData[LLML_ROTATE_DEG_ID].value.D64_Value);
        vt.append(info);

        m_line_para.theta = pRstData[LLML_THETA_ID].value.D64_Value;
        m_line_para.degree = pRstData[LLML_DEGREE_ID].value.D64_Value;
        m_line_para.rotate_deg = pRstData[LLML_ROTATE_DEG_ID].value.D64_Value;
        m_line_para.cos = pRstData[LLML_COS_ID].value.D64_Value;
        m_line_para.sin = pRstData[LLML_SIN_ID].value.D64_Value;
        m_line_para.x = pRstData[LLML_POINT_X_ID].value.D64_Value;
        m_line_para.y = pRstData[LLML_POINT_Y_ID].value.D64_Value;
        m_line_para.phy_x = pRstData[LLML_PHY_POINT_X_ID].value.D64_Value;
        m_line_para.phy_y = pRstData[LLML_PHY_POINT_Y_ID].value.D64_Value;
