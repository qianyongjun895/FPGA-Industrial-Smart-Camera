...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    pRunPara = (COLOR_AREA_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);
...
//    if(pRstData->resultflag)//该数据有效
    {
        info.strName = "面积:";
        info.strValue = QString::number(pRstData[COLOR_AREA].value.U32_Value);
        vt.append(info);
    }
    return vt;
}
