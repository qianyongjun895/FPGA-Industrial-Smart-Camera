...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    P2L_DIST_OUTPUT_PARAM *pRunPara =  (P2L_DIST_OUTPUT_PARAM *)Task_Step_Output_Para_Get(step);
    STRU_KEY_VALUE info;
    info.strName = "任务名称:点线距离";
    info.strValue = "\r\n";
    vt.append(info);
...
//    if(pRstData->resultflag)//该数据有效
    {
        info.strName = "像素距离:";
        info.strValue = QString::number(pRstData[PLD_POINT_LINE_DIST_ID].value.D64_Value);
        vt.append(info);
        info.strName = " ";
        info.strValue =" ";
        vt.append(info);
        info.strName = "物理距离:";
        info.strValue = QString::number(pRstData[PHY_PLD_POINT_LINE_DIST_ID].value.D64_Value);
        vt.append(info);
        info.strName = " ";
        info.strValue =" ";
        vt.append(info);
        ...
        int pix_real_x = 0;
        int pix_real_y = 0;
        float pix_sub_x = pRstData[PLD_STROKE_POINT_X_ID].value.F32_Value;
        float pix_sub_y = pRstData[PLD_STROKE_POINT_Y_ID].value.F32_Value;
