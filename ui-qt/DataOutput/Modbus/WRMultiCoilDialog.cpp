...
#include "ui_WRMultiCoilDialog.h"
#include <QDebug>
#include <QMessageBox>
...
WRMultiCoilDialog::WRMultiCoilDialog(QWidget *parent) :
    QDialog(parent),
    ui(new Ui::WRMultiCoilDialog)
{
    ui->setupUi(this);
    Ini_Flag = 0;
    LoadKeyBoardLib();
...
WRMultiCoilDialog::~WRMultiCoilDialog()
{
...
void WRMultiCoilDialog::IniData()
{
    Page_cnt = 1;
    current_page = 1;
...
void WRMultiCoilDialog::IniMultiCoilData()
{
...
void WRMultiCoilDialog::IniSlaveWriteCoilData()
{
    modbus_data_list_get(Data_List_Index, &WR_Coil_Data);
...
void WRMultiCoilDialog::IniSlaveWriteDiscreteData()
{
    modbus_data_list_get(Data_List_Index, &WR_Discrete_Data);
...
void WRMultiCoilDialog::IniHostReadCoilData()
{
    modbus_data_list_get(Data_List_Index, &H_RD_Coil_Data);
...
void WRMultiCoilDialog::IniHostWriteCoilData()
{
    modbus_data_list_get(Data_List_Index, &H_WR_Coil_Data);
...
void WRMultiCoilDialog::IniHostReadDiscreteData()
{
    modbus_data_list_get(Data_List_Index, &H_RD_Discrete_Data);
...
void WRMultiCoilDialog::GetMultiCoilData()
{
    Ini_Flag = 1;
...
void WRMultiCoilDialog::SetModbusMode(int mode)
{
    slave_or_host = mode;
}

void WRMultiCoilDialog::SetDataListIndex(int List_Index)
{
    Data_List_Index = List_Index;
}

void WRMultiCoilDialog::SetToolType(int tool)
{
    tool_type = tool;
}

void WRMultiCoilDialog::LoadKeyBoardLib()
{
...
void WRMultiCoilDialog::NumKeyBoardDlg(QObject *watched, QEvent *event, int Min_value, int Max_value)
{
...
bool WRMultiCoilDialog::eventFilter(QObject *watched, QEvent *event)
{
...
void WRMultiCoilDialog::AddMultiColiItem(MultiCoilItem* CoilItem)
{
...
void WRMultiCoilDialog::CoilCofingChanged(int number, int coil_addr, int data_cnts)
{
    ui->listWidget->clear();
    for(int i = 1; i <= data_cnts; i++)
    {
        MultiCoilItem *item = new MultiCoilItem;
...
void WRMultiCoilDialog::on_lineEdit_StartAddr_textChanged(const QString &arg1)
{
    CoilSetChanged();
}

void WRMultiCoilDialog::on_lineEdit_Length_textChanged(const QString &arg1)
{
    CoilSetChanged();
    CalcPageData();
}

void WRMultiCoilDialog::CoilSetChanged()
{
    if(Ini_Flag == 0)
    {
        memset(&WR_Coil_Data, 0, sizeof(WR_Coil_Data));
...
void WRMultiCoilDialog::CalcPageData()
{
    int length = ui->lineEdit_Length->text().toInt();
    if(length > 10)
    {
...
void WRMultiCoilDialog::on_btnPrePage_clicked()
{
    if(current_page == 1)
    {
        return;
...
void WRMultiCoilDialog::on_btnNextPage_clicked()
{
...
void WRMultiCoilDialog::on_btnOk_clicked()
{
    if(slave_or_host == MODBUS_SLAVE_TYPE)
    {
        if(tool_type == SLAVE_WR_COILS)
        {
            ...
            modbus_data_list_set(slave_or_host, Data_List_Index, &WR_Coil_Data, tool_type);
        }
        else if(tool_type == SLAVE_WR_DISCRETE)
        {
            WR_Discrete_Data.address = ui->lineEdit_StartAddr->text().toUInt();
            WR_Discrete_Data.length = ui->lineEdit_Length->text().toUInt();
            modbus_data_list_set(slave_or_host, Data_List_Index, &WR_Discrete_Data, tool_type);
        }
    }
    else
    {
        if(tool_type == HOST_RD_COILS)
        {
            ...
            modbus_data_list_set(slave_or_host, Data_List_Index, &H_RD_Coil_Data, tool_type);
        }
        else if(tool_type == HOST_WR_COILS)
        {
            H_WR_Coil_Data.address = ui->lineEdit_StartAddr->text().toUInt();
            H_WR_Coil_Data.length = ui->lineEdit_Length->text().toUInt();
            modbus_data_list_set(slave_or_host, Data_List_Index, &H_WR_Coil_Data, tool_type);
        }
        else if(tool_type == HOST_RD_DISCRETE)
        {
            H_RD_Discrete_Data.address = ui->lineEdit_StartAddr->text().toUInt();
            H_RD_Discrete_Data.length = ui->lineEdit_Length->text().toUInt();
            modbus_data_list_set(slave_or_host, Data_List_Index, &H_RD_Discrete_Data, tool_type);
        }
    }
...
void WRMultiCoilDialog::on_btnquit_clicked()
{
    QDialog::reject();
}
