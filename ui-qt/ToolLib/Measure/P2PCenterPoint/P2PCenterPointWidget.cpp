#include <QDebug>
#include "P2PCenterPointWidget.h"
#include "ui_P2PCenterPointWidget.h"
...
P2PCenterPointWidget::P2PCenterPointWidget(QWidget *parent) :
    QWidget(parent),
    ui(new Ui::P2PCenterPointWidget)
{
    ui->setupUi(this);
    m_label = new FWP2PCenterPointLabel(this);
...
P2PCenterPointWidget::~P2PCenterPointWidget()
{
    if(m_label !=NULL)
    {
        delete m_label;
...
QPoint P2PCenterPointWidget::Get_Image_Pos()
{
    QPoint point(10,10);
...
/**
 * @brief P2PCenterPointWidget::Set_Parameter_To_Ram
 * @author dgq
 * @note 将界面的配置参数写入内存
 */
int P2PCenterPointWidget::Set_Parameter_To_Ram()
{
...
/**
 * @brief P2PCenterPointWidget::Init_Input_Ptr
 * @param ptr 输入结构体内存指针
 * @param i_step_index 任务步骤号
 * @author dgq
 * @note 初始化Input结构体指针
 */
void P2PCenterPointWidget::Init_Input_Ptr(void *ptr,int i_step_index,int new_flag,void *pen_color)
{
    ((FWP2PCenterPointLabel*)m_label)->SetPenColor(pen_color);
    pInputPara = ptr;
...
QString P2PCenterPointWidget::InitPointCombobox(int TaskIndex, int id)
{
    QString name;
 ...
 * @note 初始化
 */
void P2PCenterPointWidget::InitData(int i_step_index)
{
    int i = 0;
    TASK_STEP_HEADER *pStephead = Task_Step_Head_Get(i_step_index);
    ...
    ((FWP2PCenterPointLabel*)m_label)->SetPoint1(point);
    ...
    ((FWP2PCenterPointLabel*)m_label)->SetPoint2(point);

}


/**
 * @brief P2PCenterPointWidget::Point1TaskChangeSlot
 * @param index 下拉框的当前索引
 * @author dgq
 * @note 点1的关联任务发生改变时的处理
 */
void P2PCenterPointWidget::Point1TaskChangeSlot(int index)
{
    ((FWP2PCenterPointLabel*)m_label)->SetPoint1(QPoint(-20,-20));
    if(m_i_load_first == 1)
    {
        ...
        else
        {
            ...
            ((FWP2PCenterPointLabel*)m_label)->SetPoint1(point1);
        }
    }
}

/**
 * @brief P2PCenterPointWidget::Point2TaskChangeSlot
 * @param index 下拉框的当前索引
 * @author dgq
 * @note 点2的关联任务发生改变时的处理
 */
void P2PCenterPointWidget::Point2TaskChangeSlot(int index)
{
    ((FWP2PCenterPointLabel*)m_label)->SetPoint2(QPoint(-20,-20));
    if(m_i_load_first == 1)
    {
        ...
        else
        {
            ...
            ((FWP2PCenterPointLabel*)m_label)->SetPoint2(point2);
        }
...
/**
 * @brief P2PCenterPointWidget::Point1ResultChangeSlot
 * @param index 下拉框索引
 * @author dgq
 * @note 引用结果发生改变时的处理
 */
void P2PCenterPointWidget::Point1ResultChangeSlot(int index)
{
    if(m_i_load_first ==1)
    {
...
void P2PCenterPointWidget::Point2ResultChangeSlot(int index)
{
    if(m_i_load_first ==1)
    {
...
void P2PCenterPointWidget::GetRefPara(int ref_step,int ref_result,QPoint *point)
{
...
void P2PCenterPointWidget::Get_Result_From_Ram()
{
    PPMP_PRIVATE_PARAM *pPrivate = (PPMP_PRIVATE_PARAM *)Task_Step_Private_Para_Get(m_step_index);

    int pix_vga_x = 0;
    int pix_vga_y = 0;
    float pix_sub_x = pPrivate->model_ppmp_result.point_pixel.x_f;
    float pix_sub_y = pPrivate->model_ppmp_result.point_pixel.y_f;
    PixelCvt(&pix_sub_x,&pix_vga_x,CVT_SUB2VGA|CVT_F2I|CVT_DATA);
    PixelCvt(&pix_sub_y,&pix_vga_y,CVT_SUB2VGA|CVT_F2I|CVT_DATA);
    ((FWP2PCenterPointLabel*)m_label)->SetSet_Parameter_To_RamPoint(QPoint(pix_vga_x,pix_vga_y));
}
