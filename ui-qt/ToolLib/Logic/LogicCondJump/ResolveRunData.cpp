...
QVector<STRU_KEY_VALUE> ResolveRunData::GetHead(int step)
{
    ...
    if(((CONDITION_JUMP_OUTPUT_PARAM*)pRunPara)->common_param.err_type_id != 0)
    {
        info.strName = "执行结果:";
        info.strValue = "NG";
    }else
    ...
    vt.append(info);
    info.strName = "OK计数:";
    info.strValue = QString::number(((CONDITION_JUMP_OUTPUT_PARAM*)pRunPara)->common_param.OK_count);
    vt.append(info);
    info.strName = "NG计数:";
    info.strValue = QString::number(((CONDITION_JUMP_OUTPUT_PARAM*)pRunPara)->common_param.NG_count);
    vt.append(info);
    info.strName = "运行时间:";
    info.strValue = QString::number(((CONDITION_JUMP_OUTPUT_PARAM*)pRunPara)->common_param.step_cost_time)+"ms";
    vt.append(info);
    return vt;
}
...
//        vt.append(info);
        info.strName = "跳转步骤编号:";
        info.strValue = QString::number(pRstData[COND_JUMP_STEP].value.U32_Value);
        vt.append(info);
//    }
    return vt;
}
